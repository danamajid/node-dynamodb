...
Formatter.prototype.getItemOutput = function(callback) {
  ...
  return function(err, resp) {
    ...
    if (resp && resp.Item) {
      return callback(null, self._fromDynamoDB(resp.Item));
    } else {
...
Formatter.prototype.batchPutItemInput = function(tableName, rawItems, schema) {
  var operation = {
    RequestItems: {}
  };
  ...
  for (var i in rawItems) {
    var put = {
      PutRequest: {
        Item: this._toDynamoDB(rawItems[i], schema)
      }
    };
  ...
  operation.RequestItems[tableName] = items;
  return operation;
};

Formatter.prototype.batchPutItemOutput = function(callback) {
  return callback;
};
