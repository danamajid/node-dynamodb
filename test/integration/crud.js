  ...
  describe('Batch PUT', function() {
    it('Should be able to handle empty put or missing range / index key', function(done) {
      ...
      async.each(
        cases,
        function(input, next) {
          Movie.batchPut(input, function(err, result) {
            expect(err).to.have.property('code');
            expect(err.code).to.equal('ValidationException');
            expect(err).to.have.property('message');
            expect(err.message).to.equal('One of the required keys was not given a value');
    ...
    it('Should be able to handle input not matching the field data type', function(done) {
      var cases = [
        ...
        },
        {
          ...
          expectation: {
            code: 'InvalidParameterType',
            message: 'Expected params.RequestItems[\'dev.biem.Movies\'][0].PutRequest.Item[\'year\'].N to be a string',
            extraProperties: false
          }
        }
      ];
      ...
      async.each(
        cases,
        function(testingCase, next) {
          Movie.batchPut(testingCase.input, function(err, result) {
            expect(err).to.not.be.a('null');
            expect(err).to.have.property('code');
            expect(err.code).to.equal(testingCase.expectation.code);
            expect(err).to.have.property('message');
      ...
      async.each(
        ...
        function(input, next) {
          Movie.batchPut(input, function(err, result) {
            ...
            expect(Object.keys(result.UnprocessedItems)).to.have.length(0);
            next();
          });
        }, done
      );
